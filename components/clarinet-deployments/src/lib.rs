...
pub async fn generate_default_deployment_with_cache(
    ...
    // Build the ASTs / DependencySet for requirements - step required for Simnet/Devnet/Testnet/Mainnet
    {
        ...
        // Set of contract IDs that already exist at a network-specific address.
        // For testnet, when entry.testnet differs from entry.contract_id we skip
        // creating a RequirementPublish — the identifier is rewritten in source
            ...
            let (clarity_version, ast) = match requirements_data.remove(&contract_id) {
                ...
                None => {
                    ...
                    if matches!(network, StacksNetwork::Simnet) {
                        ...
                        // If this contract has a network-specific testnet address
                        // in the address_map, skip creating a RequirementPublish —
                        // the identifier will be rewritten in source text at
                        // broadcast time via contracts_ids_to_remap.
