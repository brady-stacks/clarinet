...
pub struct ProjectConfigFile {
    ...
    /// External contract dependencies (legacy — merged into address_map)
    #[serde(default)]
    #[cfg_attr(
        feature = "json_schema",
...
pub struct AddressMapEntry {
    ...
    /// Override address for devnet deployments (optional)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub devnet: Option<String>,
    /// Override address for testnet deployments (optional — when set and different
    /// from `contract_id`, Clarinet remaps references at broadcast time rather than
    /// re-deploying the contract)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub testnet: Option<String>,
    /// Override address for mainnet deployments (optional)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub mainnet: Option<String>,
}

/// Backward-compatible alias — `[[project.requirements]]` entries are parsed
/// into `AddressMapEntry` values with only `contract_id` populated.
...
impl ProjectManifest {
    ...
    fn register_contract(
        ...
        let (epoch, parsed_clarity_version) =
    ...
    pub fn from_project_manifest_file(
        ...
        // Parse legacy `[[project.requirements]]` — each entry becomes an
        // AddressMapEntry with only `contract_id` set.
        ...
        // Parse `[[project.address_map]]` — superset of requirements. If the
        // same contract_id appears in both sections the address_map entry wins
        // (it may carry per-network overrides) and replaces the requirements
        // entry in-place to preserve ordering.
        if let Some(TomlValue::Array(entries)) = project_manifest_file.project.address_map {
            for item in entries.iter() {
                if let TomlValue::Table(table) = item {
                    ...
                    if !seen_ids.insert(contract_id.clone()) {
                        // Already added from requirements — replace in-place so
...
fn get_epoch_and_clarity_version(
    ...
    if clarity_version > ClarityVersion::default_for_epoch(epoch.resolve()) {
...
mod tests {
    ...
    fn test_get_epoch_and_clarity_version() {
        ...
        assert_eq!(result, Ok((Specific(Epoch20), Clarity1)));
        ...
        assert_eq!(result, Ok((Specific(Epoch21), Clarity2)));

        // epoch 3.0, no version
        let result = get_epoch_and_clarity_version(Some("3.0"), None);
        assert_eq!(result, Ok((Specific(Epoch30), Clarity3)));
        ...
        assert_eq!(result, Ok((Specific(Epoch21), Clarity1)));
    ...
    #[test]
    fn test_epoch_latest_preservation() {
        let manifest_toml = toml! {
            [project]
            name = "test-project"
            telemetry = false
        ...
        assert!(matches!(contract.epoch, clarity_repl::repl::Epoch::Latest));
        let toml_value = toml::Value::try_from(&manifest).unwrap();
        let pretty_toml = toml::ser::to_string_pretty(&toml_value).unwrap();
        assert!(pretty_toml.contains("epoch = 'latest'"));
    }
    ...
    fn test_requirements_backward_compat() {
        // Legacy [[project.requirements]] entries are parsed into address_map
        // with only contract_id set and no network overrides.
        let manifest_str = r#"
[project]
name = "test-project"
telemetry = false
...
"#;
    ...
    fn test_address_map_parsing() {
        // [[project.address_map]] entries carry per-network overrides.
        let manifest_str = r#"
[project]
name = "test-project"
telemetry = false
...
"#;
    ...
    #[test]
    fn test_address_map_wins_over_requirements() {
        // When the same contract_id appears in both sections the address_map
        // entry (with its overrides) replaces the requirements entry in-place
        // so that ordering is preserved and there are no duplicates.
        let manifest_str = r#"
[project]
name = "test-project"
telemetry = false
...
"#;
        ...
        // Duplicate removed — 2 entries, not 3.
        assert_eq!(manifest.project.address_map.len(), 2);
        ...
        // Position preserved: nft-trait is first (came from requirements first).
        assert_eq!(
            manifest.project.address_map[0].contract_id,
            "SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait"
        );
