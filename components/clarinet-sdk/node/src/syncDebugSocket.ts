...
export async function connectSyncSocket(port: number): Promise<void> {
  ...
  const s: WorkerState = { worker, header, signal, data, dead: false, cause: null };
  ...
  worker.on("exit", (code) => {
    s.dead = true;
    if (!s.cause && code !== 0) s.cause = new Error(`debug socket worker exited with code ${code}`);
  });
...
export function syncSend(request: Record<string, unknown>): string {
  ...
  if (state.dead) {
    throw new Error("debug socket worker died during request: " + (state.cause?.message ?? "unknown"));
  }
