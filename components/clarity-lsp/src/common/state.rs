    ...
    pub fn get_code_lenses(&self, contract_location: &FileLocation) -> Vec<lsp_types::CodeLens> {
        ...
        for (function_name, range) in &contract_state.definitions {
            ...
            if let Some((cost_node, _)) = cost_analysis.get(&function_name_str) {
                ...
                // Create a command to open cost details view
                let contract_uri = contract_location.to_url_string().ok();
                let arguments: Option<Vec<serde_json::Value>> = contract_uri.map(|uri| {
                    vec![serde_json::json!({
                        "path": uri,
                        "line": code_lens_line + 1, // LSP uses 1-based line numbers
